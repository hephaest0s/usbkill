...
help_message = ("usbkill is a simple program with one goal: quickly shutdown "
                "the computer when a usb is inserted or removed.\nIt logs to "
                "/var/log/usbkill/kills.log\nYou can configure a whitelist "
                "of usb ids that are acceptable to insert and the remove."
                "\nThe usb id can be found by running the command 'lsusb'."
                "\nSettings can be changed in /etc/usbkill/settings\n\nIn "
                "order to be able to shutdown the computer, this program "
...
def kill_computer():
    ...
    if current_platform.startswith("DARWIN"):
        ...
        # Use Kernel Panic instead of shutdown
        # command (30% faster and encryption keys are released)
...
def lsusb():
    # A python version of the command 'lsusb'
    # that returns a list of connected usbids
...
def settings_template(filename):
    ...
    if not os.path.isfile(filename):
        ...
        with open(filename, 'w') as f:
            f.write("# whitelist command lists the usb ids that you want "
                    "whitelisted\n")
            f.write("# find the correct usbid for your trusted usb using the "
                    "command 'lsusb'\n")
            f.write("# usbid looks something line 0123:9abc\n")
            f.write("# Be warned! other parties can copy your trusted usbid "
                    "to another usb device!\n")
            f.write("# use whitelist command and single space separation "
                    "as follows:\n")
            f.write("# whitelist usbid1 usbid2 etc\n")
            f.write("whitelist \n\n")
            f.write("# allow for a certain amount of sleep time between "
                    "checks, e.g. 0.5 seconds:\n")
            f.write("sleep 0.5\n")
...
def load_settings(filename):
    ...
    assert None not in [devices, sleep_time], (
        "Please set the 'sleep' and "
        "'whitelist' parameters in '/etc/usbkill/settings' !")
    assert sleep_time > 0.0, (
        "Please allow for positive non-zero 'sleep' "
        "delay between usb checks!")
    return devices, sleep_time


def loop(whitelisted_devices, sleep_time):
    # Main loop that checks every 'sleep_time' seconds
    # if computer should be killed.
    # Allows only whitelisted usb devices to connect!
    # Does not allow usb device that was present during program
    # start to disconnect!
    start_devices = lsusb()
    acceptable_devices = set(start_devices + whitelisted_devices)

    # Write to logs that loop is starting:
    msg = ("Started patrolling the "
           "USB ports every " + str(sleep_time) + " seconds...")
    log(msg)
    print(msg)
